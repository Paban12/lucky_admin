...
function PlayerTab({ gameName = 'balckandwhite' }) {
  ...
  const filteredUsers = gameHistoryData.sort((a, b) => new Date(b.dateTime) - new Date(a.dateTime)).filter((user) => {
  ...
  const formatDate = (isoDateString) => {
    ...
    const month = String(istDate.getMonth() + 1).padStart(2, '0'); // Months are 0-based in JS
    ...
    hours = String(hours).padStart(2, '0'); // Ensure two-digit hours
            ...
            {usersOnCurrentPage?.map((user, index) =>
              pageSize
                ? index + 1 <= pageSize && (
                  <CustomerInfo
                    key={user._id}
                    datetime={formatDate(user.dateTime)}
                    UserName={user.userData?.name}
                    MobileNo={user.userData?.mobileNumber}
                    roomid={user.gameId}
                    amount={user.betAmount}
