...
function PlayerTab({ UserId, gameName }) {
  ...
  useEffect(() => {
    const submitdata = async () => {
      ...
      } else if (gameName == "Black and White") {
        let res = await GetBlackandWhiteHistoryData(Botinfo.UserId)
  ...
  if (gameHistoryData && gameHistoryData.length > 0) {
    filteredUsers = gameHistoryData.sort((a, b) => new Date(b.dateTime) - new Date(a.dateTime)).filter((user) => {
              ...
              pageSize
                ? index + 1 <= pageSize && (
                  <CustomerInfo
                    ...
                    roomid={user.userData?.roomId}
                    amount={user.betAmount}
                    amountWin={user.winAmount}
                    type={user.type}
                    club={"BlackNWhite"}
