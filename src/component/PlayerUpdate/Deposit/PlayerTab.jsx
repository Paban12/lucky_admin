...
function PlayerTab({ UserId, gameName }) {
  ...
  const { GetCompleteDespositeData,GetUserDespositeData, GetRegisterReferralBonusData } = context
  ...
  useEffect(() => {
    const submitdata = async () => {
      ...
      if (gameName == "Referral Bonus") {
        ...
        let res=await GetUserDespositeData(Botinfo.UserId)
        ...
        let res=await GetUserDespositeData(Botinfo.UserId)
        setGameHistoryData(res)
        console.log('gameHistoryData GetUserDespositeData',{res});
              ...
              pageSize
                ? index + 1 <= pageSize && (
                  <CustomerInfo
                    ...
                    oppChips={user.userId?.mobileNumber}
                    trnxAmount={user.transactionId}
                    totalBucket={user.amount}
                    trnxTypeTxt={user.status}
